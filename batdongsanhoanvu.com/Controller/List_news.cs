...
namespace Controller
{
    public class List_news
    {
        ...
        public string GetCateSeoParent(string _cat_seo_url)
        {
            try
            {
                var list = db.ESHOP_CATEGORies.Where(n => n.CAT_SEO_URL == _cat_seo_url).Select(n => new { n.CAT_PARENT_ID }).ToList();
                if (list.Count > 0)
                {
                    var list2 = db.ESHOP_CATEGORies.Where(n => n.CAT_ID == list[0].CAT_PARENT_ID).Select(n => new { n.CAT_SEO_URL }).ToList();
                    if(list2.Count >0)
                    return list2[0].CAT_SEO_URL;
                }
                return "";
            }
            catch (Exception)
